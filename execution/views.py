...
def get_detail_context(execution_id):
    ...
    try:
        ...
        #         f = {'lat':i, 'long':j, 'file': '{}_{}_{}_{}_output.nc'.format(execution.version.algorithm.name, execution.version.number, lat, long), 'state': False, 'tiff_file': f.replace('.nc', '.tiff')}
        #         files.append(f)
    ...
    context = {'execution': execution, 'executed_params': executed_params, 'review': review, 'files': files,
               ...
               'system_path': system_path, 'area_param':area_param.lat_start}
    return context
