...
def custom_choices():
    ...
    for su in storage_units:
        last_task = Task.objects.filter(storage_unit=su).last()
        if last_task:
            has_error = True if int(last_task.state) == int(Task.FAILED_STATED) else False
            is_available = True if int(last_task.state) == int(Task.COMPLETED_STATE) or int(
                last_task.state) == int(Task.FAILED_STATED) else False
            if has_error:
